...
- [ ] **支出一覧ページ**
  - `expenses` テーブルのレコードを一覧表示
  - 表示項目: 取引日 (`transaction_date`), 金額 (`amount`), 取引先 (`merchant`), カテゴリ (`category_id`)
  - 年ごとの表示（ダッシュボードと連動）
  - **検索機能**: 取引先 (`merchant`) での絞り込み表示
  - **ソート機能**: 取引日、金額などでのソート（デフォルトは取引日降順）
  - **カテゴリー編集機能**: 各レコードのカテゴリーを個別に変更可能
