...
## 10. エージェントの行動規範 (Agent Development Rules)

エージェントはこのプロジェクトで作業する際、以下の規則を厳守すること。

- **自動コミットの禁止**:
  - いかなる場合も勝手にコミット（git commit）を行わない。
  - `/commit` ワークフローを使用する場合も、実行前に必ずユーザーの明示的な許可を得ること。
- **承認プロセス**:
  - 変更内容はまずコードや diff として提示し、ユーザーからの「コミットして良い」等の明示的な指示を得てからコミット手順に進む。
- **品質チェックの強制**:
  - **`apps/` または `packages/` 以下にコードの修正があった場合**、コミット前に影響範囲のプロジェクトで `pnpm lint` および `pnpm test` を実行し、エラーがないことを確認する。
  - ドキュメントのみの修正や、実装コードに影響しない root 設定ファイル（`.md` や root の設定ファイル等）の変更のみの場合は、この手順をスキップできる。
- **コマンド実行の許可**:
  - `pnpm lint` および `pnpm test` は、作業の正当性を確認するために必要な「安全なコマンド」として、ユーザーの明示的な許可なく実行（SafeToAutoRun: true）できるものとします。
- **ブランチ運用**:
  - 指定されたブランチ、または適切な feature ブランチで作業を行い、完了後に報告する。
