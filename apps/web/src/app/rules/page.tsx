...
export default function RulesPage() {
	...
	const [formData, setFormData] = useState<RuleFormData>({ keyword: '', categoryId: '', priority: 0 });
		...
		try {
			...
			if (categoriesRes.status === 200 && 'data' in categoriesRes) {
				...
				if (categoriesRes.data.data.length > 0 && !formData.categoryId) {
					setFormData((prev) => ({ ...prev, categoryId: categoriesRes.data.data[0].id }));
				}
			}
		} catch (_error) {
	...
	const handleCreate = async () => {
		...
		try {
			...
			if (response.status === 201) {
				await fetchData();
				setShowForm(false);
				setFormData({ keyword: '', categoryId: categories[0]?.id || '', priority: 0 });
					...
					<Card className="mb-6">
						...
						<CardContent>
							<div className="space-y-4">
								...
								<div className="flex gap-2">
									<button onClick={handleCreate} disabled={isSubmitting} className="px-4 py-2 bg-linear-to-r from-red-500 to-pink-600 text-white rounded-lg hover:opacity-90 disabled:opacity-50">
										{isSubmitting ? '作成中...' : '作成'}
									</button>
									<button onClick={() => setShowForm(false)} className="px-4 py-2 border rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
										キャンセル
									</button>
								</div>
							</div>
									...
									<div key={rule.id} className="py-4">
										{editingId === rule.id ? (
											<div className="space-y-4">
												...
												{error && <p className="text-sm text-red-500">{error}</p>}
												<div className="flex gap-2">
													<button onClick={() => handleUpdate(rule.id)} disabled={isSubmitting} className="p-2 text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg">
														<Check className="w-5 h-5" />
													</button>
													<button onClick={cancelEdit} className="p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg">
														<X className="w-5 h-5" />
													</button>
												</div>
											</div>
											...
											<div className="flex items-center justify-between">
												<div className="flex items-center gap-8">
													...
													<div>
														...
														<span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-sm">{rule.categoryName}</span>
													</div>
												</div>
												<div className="flex gap-1">
													<button onClick={() => startEdit(rule)} className="p-2 text-gray-500 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors">
