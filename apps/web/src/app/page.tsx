...
export default function Dashboard() {
	...
	// 先月の支出を計算
	const lastMonthExpense = summary?.monthlyBreakdown?.find((m: MonthlyBreakdown) => m.month === currentMonth - 1)?.totalAmount ?? 0;
	...
	const topCategory = summary?.categoryBreakdown?.reduce(
		(max: CategoryBreakdown | null, current: CategoryBreakdown) => (max === null || current.totalAmount > max.totalAmount ? current : max),
