...
export const categories = pgTable(
	...
	{
		id: pgUuid('id').defaultRandom().primaryKey(),
		userId: char('user_id', { length: 36 }).references(() => users.uuid, { onDelete: 'cascade' }),
...
export const expenses = pgTable(
	'expenses',
	{
		...
		userId: char('user_id', { length: 36 })
			.notNull()
			.references(() => users.uuid, { onDelete: 'cascade' }),
		uploadId: pgUuid('upload_id').references(() => csvUploads.id, { onDelete: 'set null' }),
		transactionDate: timestamp('transaction_date').notNull(),
		amount: integer('amount').notNull(),
		merchant: varchar('merchant', { length: 200 }).notNull(),
		categoryId: pgUuid('category_id').references(() => categories.id, { onDelete: 'set null' }),
